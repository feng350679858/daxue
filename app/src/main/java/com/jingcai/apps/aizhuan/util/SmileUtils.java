    ...
    public static void backspace(EditText editText){
        ...
        if (selectionStart > 0) {
            ...
            if (!TextUtils.isEmpty(body)) {
                ...
                if (i != -1) {
                    CharSequence lastUnit = tempStr.subSequence(i, selectionStart);
                    String quotedUnit =  Pattern.quote(lastUnit.toString());
                    boolean matched = false;
                    Set<Pattern> patterns = emoticons.keySet();
                    for(Pattern p : patterns){
                        if(p.pattern().equals(quotedUnit)){
                            matched = true;
                            break;
                        }
                    }
