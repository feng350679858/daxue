...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void init(Context context) {
        mContext = context;
        appName = mContext.getString(R.string.app_name_zh);
...
//                        }).show();
        ...
        new Thread() {
            ...
            public void run() {
                ...
                // 保存日志文件
                String errmsg = saveCrashInfo2File(ex, infos);
                ...
                Notification newNotification = new Notification(R.drawable.ic_launcher, "系统出错,点击查看详情！", System.currentTimeMillis());
                Intent baseIntent = new Intent(mContext, ErrorActivity.class);
                baseIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                baseIntent.setAction(String.valueOf(System.currentTimeMillis()));
                Bundle bundle = new Bundle();
                bundle.putString("msg", errmsg);
                baseIntent.putExtras(bundle);

                PendingIntent contentIntent = PendingIntent.getActivity(
                        mContext, 0, baseIntent,
                        PendingIntent.FLAG_UPDATE_CURRENT);

                newNotification.contentIntent = contentIntent;
                newNotification.flags = Notification.FLAG_AUTO_CANCEL;
                newNotification.setLatestEventInfo(mContext, appName, "系统出错,点击查看详情！", contentIntent);

                mNotificationManager.notify(1, newNotification);
